  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center bg-black text-white overflow-hidden px-3 sm:px-4">
      ...
      <section className="text-center mt-16 md:mt-24 z-10 w-full max-w-6xl pb-6 md:pb-12 px-2 sm:px-4">
          ...
          {sections.map((section, index) => (
            ...
            >
              ...
              <div className="flex items-center mb-2 sm:mb-4">
                <span
                  className={`text-xl sm:text-3xl mr-2 sm:mr-3 text-${section.color}-400`}
                >
                  {section.icon}
                </span>
                <h2
                  className={`text-lg sm:text-2xl font-bold font-mono text-${section.color}-400`}
                >
                  {section.title}
                </h2>
              </div>

              <p className="text-xs sm:text-sm text-gray-300 mb-3 sm:mb-6 font-mono leading-tight">
                {section.description}
              </p>
                ...
                {section.links.map((link, i) => (
                  <li key={i}>
                    <a
                      href={link.url}
                      className={`
                        block px-2 py-1 sm:py-2 text-xs font-mono 
                    ...
                    >
                      <span
                        className={`text-${section.color}-400 mr-1 sm:mr-2`}
                      >
                        ▸
                      </span>
                      {link.name}
              ...
              {/* Hover Effect */}
              <div
                className={`absolute inset-0 -z-10 opacity-0 group-hover:opacity-100 bg-gradient-to-br from-${section.color}-900/10 to-black/80 transition-opacity duration-500`}
              ></div>
            </div>
          ))}
        </div>
        ...
        <div
          className={`mt-6 sm:mt-16 p-3 sm:p-6 rounded-lg border border-white/10 max-w-3xl mx-auto ${styles.glassEffect}`}
        >
          <p className="text-[10px] sm:text-sm text-gray-400 font-mono">
