    ...
    return (
        <main className="relative min-h-screen flex flex-col items-center bg-black text-white overflow-hidden px-4 sm:px-6">
            ...
            <section className="text-center mt-20 md:mt-24 z-10 w-full max-w-6xl pb-8 md:pb-12">
                {/* Responsive heading */}
                <h1 className={`text-3xl sm:text-4xl md:text-5xl font-bold text-red-400 ${styles.glowRed} font-mono mb-6 md:mb-8`}>
                    <FuzzyText baseIntensity={0.1} hoverIntensity={0.3} color='red' fontSize="clamp(32px, 6vw, 48px)">
                        Neural Nexus
                    </FuzzyText>
                </h1>

                <p className="text-sm sm:text-base md:text-lg text-gray-400 font-mono mb-4 md:mb-6 max-w-2xl mx-auto">
                    (root@0xpwns3c)-[~/blog] # access_cyber_consciousness
                </p>

                {/* Interactive Terminal */}
                <div className="mb-6 md:mb-8 p-3 sm:p-4 rounded-lg bg-black border border-green-400/30 shadow-lg">
                    <div className="flex items-center font-mono text-xs sm:text-sm text-green-400 mb-1 sm:mb-2">
                        <span className="mr-2">┌─(user@0xpwns3c)-[~/blog]</span>
                ...
                <div className="flex flex-col sm:flex-row justify-center gap-2 sm:gap-4 mb-6 md:mb-8">
                    <button
                        ...
                        className={`px-3 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-mono border ${viewMode === 'grid' ? 'border-red-400 text-red-400' : 'border-white/20 text-white/50'}`}
                    >
                        GRID_VIEW
                    </button>
                    <button
                        onClick={() => setViewMode('neural')}
                        className={`px-3 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-mono border ${viewMode === 'neural' ? 'border-blue-400 text-blue-400' : 'border-white/20 text-white/50'}`}
                    >
                        NEURAL_MAP
                    </button>
                </div>
                ...
                {viewMode === 'grid' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        {posts.map((post) => (
                            <div
                                key={post.id}
                                className={`p-4 sm:p-6 rounded-lg border ${post.encrypted ? 'border-yellow-400/30' : 'border-white/20'} hover:border-red-400 transition-all ${styles.glassEffect} backdrop-blur-md cursor-pointer`}
                                onClick={() => setActivePost(post)}
                            >
                                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2 sm:mb-3 gap-1 sm:gap-0">
                                    <h2 className={`text-base sm:text-lg md:text-xl font-bold font-mono ${post.encrypted ? 'text-yellow-400' : 'text-red-400'}`}>
                                        {post.encrypted ? '[ENCRYPTED] ' : ''}{post.title}
                                    </h2>
                                    <span className="text-xs text-gray-400">{post.date}</span>
                                </div>
                                <p className="text-xs sm:text-sm text-gray-300 mb-3 sm:mb-4">{post.excerpt}</p>
                                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
                                    <div className="flex flex-wrap gap-1 sm:gap-2">
                                        {post.tags.map(tag => (
                                            <span key={tag} className="text-xs font-mono text-blue-400">{tag}</span>
                                        ))}
                                    </div>
                                    <span className="text-xs font-mono text-green-400">{post.views} views</span>
                                </div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-lg overflow-hidden border border-blue-400/30 backdrop-blur-md">
                            ...
                            <div className="relative w-full h-full">
                                {posts.map((post, i) => {
                                    ...
                                    return (
                                        <div
                                            key={post.id}
                                            className={`absolute w-6 h-6 sm:w-8 sm:h-8 rounded-full ${post.encrypted ? 'bg-yellow-400/30 border border-yellow-400' : 'bg-blue-400/30 border border-blue-400'} flex items-center justify-center cursor-pointer animate-pulse`}
                                            style={{ left: `${x}%`, top: `${y}%` }}
                                            onClick={() => setActivePost(post)}
                                        >
                                            <span className="text-[8px] sm:text-xs font-mono">0x{post.id}</span>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="absolute bottom-2 sm:bottom-4 left-0 right-0 text-center font-mono text-blue-400 text-xs sm:text-sm">
                ...
                {activePost && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 backdrop-blur-lg">
                        <div
                            className={`relative max-w-full w-full sm:max-w-3xl p-4 sm:p-6 md:p-8 rounded-lg bg-black/90 border ${activePost.encrypted ? 'border-yellow-400' : 'border-red-400'} shadow-2xl`}
                            onClick={(e) => e.stopPropagation()}
                        >
                            <button
                                onClick={() => setActivePost(null)}
                                className="absolute top-2 right-2 sm:top-4 sm:right-4 text-gray-400 hover:text-white text-xs sm:text-sm"
                            >
                                [X_CLOSE]
                            </button>

                            <h2 className={`text-xl sm:text-2xl md:text-3xl font-bold font-mono mb-1 sm:mb-2 ${activePost.encrypted ? 'text-yellow-400' : 'text-red-400'}`}>
                                {activePost.encrypted ? '🔒 ENCRYPTED CONTENT 🔒' : activePost.title}
                            </h2>

                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-6 gap-1 sm:gap-0">
                                <span className="font-mono text-xs sm:text-sm text-gray-400">by {activePost.author}</span>
                                <span className="font-mono text-xs sm:text-sm text-gray-400">{activePost.date}</span>
                            </div>
                                ...
                                <>
                                    ...
                                    <div className="flex flex-wrap gap-1 sm:gap-2 mb-4 sm:mb-6">
                                        {activePost.tags.map(tag => (
                                            <span key={tag} className="px-2 py-1 text-xs font-mono bg-blue-400/10 text-blue-400 rounded">
                                                {tag}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pt-3 sm:pt-4 border-t border-white/20 gap-2 sm:gap-0">
                                <span className="font-mono text-xs sm:text-sm text-green-400">{activePost.views} views</span>
                                <button className="px-3 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-mono border border-white/20 hover:border-red-400 hover:text-red-400 transition-all">
                                    DOWNLOAD_POST
                                </button>
                            </div>
                        </div>
            ...
            <footer className="mt-auto py-4 sm:py-6 w-full text-center text-gray-500 text-xs sm:text-sm font-mono">
                <p>(root@0xpwns3c)-[~/blog] # <span className={styles.typingEffect}>connection_stable</span></p>
            </footer>
        </main>
    );
}